import type { Metadata } from "next";
import "../system-software-lessons.css";
import Lesson28Client from "./lesson-28-client";

const title = "AS Computer Science · Lesson 28";
const description = "A 90-minute Cambridge 9618 lesson on IDE features and consolidation of operating systems and translators.";
const configuredSiteUrl = process.env.NEXT_PUBLIC_SITE_URL ?? "https://wenatnyu.github.io/as-course-2027/";
const siteUrl = configuredSiteUrl.endsWith("/") ? configuredSiteUrl : `${configuredSiteUrl}/`;
const lessonUrl = new URL("lesson-28/", siteUrl).toString();

export const dynamic = "force-static";

export const metadata: Metadata = {
  title,
  description,
  openGraph: { title, description, type: "website", url: lessonUrl, images: [] },
  twitter: { card: "summary", title, description, images: [] },
};

export default function Lesson28Page() {
  return <Lesson28Client />;
}
